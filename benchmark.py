...
def main():
    ...
    benchmark_results = benchmark(model_name)
    print('End Benchmark')
...
def benchmark(model_name) -> list:
    if model_name == 'MLP':
        benchmark_results = benchmark_MLP()
    elif model_name == 'CNN':
        benchmark_results = benchmark_CNN()
    else:
...
def benchmark_MLP(epochs=3) -> list:
    start = time.time()
    ...
    benchmark_results = [training_time, epochs, loss, accuracy]
    return benchmark_results

def benchmark_CNN(epochs=3) -> list:
    start = time.time()
...
def save_benchmark_results(model_name, benchmark_results, filepath) -> None:
    ...
    loss          = benchmark_results[2]
    accuracy      = benchmark_results[3]
